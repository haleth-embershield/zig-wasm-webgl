...
pub const Bird = struct {
    ...
    pub fn render(self: Bird, renderer_obj: *renderer.Renderer) void {
        ...
        renderer_obj.drawCircle(bird_x, bird_y, bird_radius, .{ 255, 255, 0 });
    }
};
...
pub const Pipe = struct {
    ...
    pub fn render(self: Pipe, renderer_obj: *renderer.Renderer) void {
        if (!self.active) return;
        ...
        // Top pipe
        if (gap_y > gap_half) {
            renderer_obj.drawRect(pipe_x, 0, pipe_width, gap_y - gap_half, .{ 0, 255, 0 });
            // Draw pipe cap
            const cap_width = @min(pipe_width + 20, GAME_WIDTH - pipe_x);
            const cap_x = if (pipe_x >= 10) pipe_x - 10 else 0;
            if (gap_y > gap_half + 15) {
                renderer_obj.drawRect(cap_x, gap_y - gap_half - 15, cap_width, 15, .{ 50, 255, 50 });
            }
        }

        // Bottom pipe
        if (gap_y + gap_half < GAME_HEIGHT) {
            renderer_obj.drawRect(pipe_x, gap_y + gap_half, pipe_width, GAME_HEIGHT - (gap_y + gap_half), .{ 0, 255, 0 });
            // Draw pipe cap
            if (pipe_x > 10) {
                const cap_width = @min(pipe_width + 20, GAME_WIDTH - pipe_x);
                const cap_x = if (pipe_x >= 10) pipe_x - 10 else 0;
                const cap_height = @min(15, GAME_HEIGHT - (gap_y + gap_half));
                renderer_obj.drawRect(cap_x, gap_y + gap_half, cap_width, cap_height, .{ 50, 255, 50 });
            }
        }
    }
